...
//  println(" typetag: "+msg.typetag());
  ...
  if(msg.checkAddrPattern("/emokit/channels")){
    ...
    for(int i=20 ; i<20+(14*4) ; i+=4){
      println("[" + i + "]: " + msg.getBytes()[i]);
    }
    println();
  } else if(msg.checkAddrPattern("/emokit/gyro")){
    println("2) Gyro:");
    // Valuable information for gyro are at bytes 20 and 24
    println("x: " + msg.getBytes()[20] + " ; y: " + msg.getBytes()[24]);
    println();
  }
}
...
public void keyPressed() {
  if (key == CODED) {
    ...
    static public void main(String args[]) {
        PApplet.main(new String[] { "--bgcolor=#ECE9D8", "osceletonARDrone" });
    }
}
